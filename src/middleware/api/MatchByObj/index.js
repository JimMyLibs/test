...
class Matches {
    ...
    async datePools() {// 根据date{pool:[matches]}划分数据
        ...
        console.log('FB_GetInfo_replacement',FB_GetInfo_replacement)
        // 一级分组：时间，二级分组：pool
        window.FB_GetInfo_replacement = FB_GetInfo_replacement;
        let poolOfData = Array.from(Object.values(FB_GetInfo_replacement.data).reduce((sum, item) => sum.concat(Object.values(item).map(item => item.pool)), []));
        console.log('poolOfData',poolOfData)
        let datePools_res = Object.values(FB_GetInfo_replacement.data).map(item1 => {
            poolOfData.map(item2 => {
                let curPoolData = Object.values(item1).filter(item3 => item2 == item3.pool);
                let tmp = {};
